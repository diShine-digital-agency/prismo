...
> **Note:** The [diShine blog article](https://dishine.it/blog/prismo-ai-consulting-toolkit-usb/) was written at the time of the initial release. The tool has evolved since then — this repository is the current reference for features, configuration, and usage.
...
**macOS / Linux:**
```bash
bash /Volumes/YOUR_USB_NAME/launch.sh     # macOS
bash /media/$USER/YOUR_USB_NAME/launch.sh  # Linux
```
Replace `YOUR_USB_NAME` with the label of your USB drive (run `ls /Volumes/` on macOS or `ls /media/$USER/` on Linux to find it).
...
├── prismo.config.json     # Configuration (agency branding, client defaults)
├── prismo-eject.ps1       # Safe USB eject (Windows)
├── VERSION                # Current version number
├── SHA256SUMS             # Tamper-detection checksums (generated by setup)
├── runtime/               # Portable Node.js + Git (downloaded by setup)
├── engine/                # AI engine + Lighthouse + pa11y (downloaded by setup)
├── config/                # Authentication and credentials (stay on the USB)
└── toolkit/
    ├── prompts/           # AI diagnostic prompt templates
    │   ├── system/        # System health, log analysis, network diagnostics
    │   ├── web/           # Performance, tech stack, accessibility
    │   ├── seo/           # Technical SEO, on-page SEO, competitive snapshot
    │   ├── martech/       # MarTech stack audit, data quality check
    │   └── security/      # Website security scan, system security audit
    ├── scripts/           # Standalone data collection scripts (see below)
    ├── reports/           # Generated audit reports (Markdown, auto-saved)
...
The `toolkit/scripts/` directory contains standalone scripts that collect raw system or web data and write it to a plain-text file — without sending anything to the AI. This is useful when you need to gather data on an air-gapped machine, or when you want to review the raw output before passing it to the AI engine.

| Script | Platform | What it collects |
|--------|----------|-----------------|
| `collect-linux.sh` | Linux | OS info, CPU, memory, disk, services, journal errors, network, open ports, firewall, SSH config, pending updates |
| `collect-macos.sh` | macOS | System info, CPU, memory, disk, SMART, network, security (SIP, Gatekeeper, FileVault), installed profiles, launch agents |
| `collect-web.sh` | Any (requires `curl`) | HTTP headers, response times, SSL certificate, meta tags, robots.txt, sitemap, security headers |
| `collect-win.ps1` | Windows | System info, running services, event log errors, network adapters, open ports, pending updates, startup items |
| `log-session.ps1` | Windows | Session logging helper (called by the Windows launcher automatically) |

**Usage:** run the script standalone, then review or feed the output file into an interactive AI session (option 14).

```bash
# Collect web data for a domain
bash toolkit/scripts/collect-web.sh https://example.com toolkit/reports/

# Collect macOS system data
bash toolkit/scripts/collect-macos.sh toolkit/reports/
```
